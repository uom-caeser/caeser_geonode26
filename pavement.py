...
try:
    from caeser_geoportal.settings import GEONODE_APPS
except:
...
def start_geoserver(options):
    ...
    from caeser_geoportal.settings import OGC_SERVER
...
def _reset():
    sh("rm -rf caeser_geoportal/development.db")
    sh("rm -rf caeser_geoportal/uploaded/*")
    _install_data_dir()
